...
export default function StationFuelTransactions() {
  /* ============================
     STATION CONTEXT
  ============================ */
  const station = useSelector((state: RootState) => state.auth.user?.station ?? undefined)
  ...
  const { data: staffData } = useStationStaff(station?.id)
  const { data: txData, isLoading: txLoading } = useStationTransactions(station?.id, query)
  const { data: summaryData } = useStationTransactionSummary(station?.id)
